...
export function SpeedReader() {
  ...
  useEffect(() => {
    ...
    if (displayMode === 'words') {
      // Конвертируем скорость из символов в минуту в слова в минуту
      // Предполагаем, что среднее слово состоит из 5 символов + пробел
      const wordsPerMinute = Math.round(speed / 6);
        ...
        <div className="w-full space-y-4 px-2 sm:px-4">
          ...
          <div>
            <label className="block mb-2 text-sm md:text-base">
              Точная настройка скорости ({displayMode === 'words' ? Math.round(speed / 6) + ' слов' : speed + ' символов'}/мин):
            </label>
            <input 
              type="range"
              min="100"
